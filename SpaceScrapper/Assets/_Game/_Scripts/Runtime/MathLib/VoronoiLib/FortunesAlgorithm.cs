...
namespace SpaceScrapper.MathLib.VoronoiLib
{
    public static class FortunesAlgorithm
    {
        public static LinkedList<VEdge> Run(List<FortuneSite> sites, double minX, double minY, double maxX, double maxY)
        {
            ...
            while (edgeNode != null)
            {
                ...
                if (!valid)
                {
                    if (edge.Left != null)
                    {
                        edge.Left.Cell.Remove(edge);
                    }
                    if (edge.Right != null)
                    {
                        edge.Right.Cell.Remove(edge);
                    }

                    TagSitesForClosing(edge);
            ...
            AssignCornersToSites(sites, minX, minY, maxX, maxY);
            CleanNeighbors(sites);

            return edges;
        }

        private static void CleanNeighbors(List<FortuneSite> sites)
        {
            foreach(var s in sites)
            {
                s.Neighbors.Clear();
                foreach(var e in s.Cell)
                {
                    s.Neighbors.Add(e.Left);
                    s.Neighbors.Add(e.Right);
                }

                s.Neighbors.Remove(s); // site not a neighbor to itself
            }
        }

        private static void AssignCornersToSites(List<FortuneSite> sites, double minX, double minY, double maxX, double maxY)
        {
            Vector2[] corners = {
                new Vector2((float)minX, (float)minY),
                new Vector2((float)minX, (float)maxY),
                new Vector2((float)maxX, (float)minY),
                new Vector2((float)maxX, (float)maxY)
            };

            FortuneSite[] cornerSites = new FortuneSite[4];
            ...
            {
                // If site needs closing, 
                if (s.ToClose)
                {
                    Vector2 vs = new Vector2((float)s.X, (float)s.Y);
                    for(int c = 0; c < corners.Length; c++)
                    {
                        float dist = Vector2.Distance(vs, corners[c]);

                        // if site is closer to corner, add site at corner position in array
                        if (dist < cornerDistances[c])
                        {
                            cornerDistances[c] = dist;
                            cornerSites[c] = s;
                        }
                    }
                }
            ...
            // go through array and assign corner to respective site
            for (int c = 0; c < corners.Length; c++)
            {
                cornerSites[c].Corners.Add(new VPoint(corners[c].x, corners[c].y));
            }
        }
        ...
        private static void TagSitesForClosing(VEdge edge)
        {
            if (edge.Left != null)
            {
                edge.Left.ToClose = true;
            }
            if (edge.Right != null)
            {
                edge.Right.ToClose = true;
            }
        }
